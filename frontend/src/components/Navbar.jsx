...
export default function Navbar() {
  ...
  const navLinks = [
    { to: '/',           label: t('nav.granciare')  },
    { to: '/olive-oil',  label: t('nav.oliveOil')   },
    { to: '/poderetto',  label: t('nav.poderetto')  },
    ...
    <header className={`navbar ${scrolled ? 'navbar--scrolled' : ''}`}>
      <div className="navbar__inner">
        ...
        <Link to="/" className="navbar__logo" onClick={handleLogoClick}>
          ...
          <span className="navbar__logo-sub">{t('nav.logoSub')}</span>
        </Link>
        ...
        <div className="navbar__actions">
          <LanguageSwitcher />
          <a href="https://wa.me/393520730769" target="_blank" rel="noreferrer" className="navbar__whatsapp">
          ...
          <button onClick={handleBookClick} className="btn btn-primary navbar__book">
            {t('nav.bookStay')}
          </button>
        </div>
      ...
      <div className={`navbar__mobile ${menuOpen ? 'navbar__mobile--open' : ''}`}>
        <nav className="navbar__mobile-links">
          ...
          <a href="https://wa.me/393520730769" target="_blank" rel="noreferrer" className="navbar__mobile-link">
            {t('nav.whatsappUs')}
          </a>
          <div className="navbar__mobile-lang">
            <LanguageSwitcher />
          </div>
          <Link to="/#book" className="btn btn-primary mt-24" onClick={() => setMenuOpen(false)}>
            {t('nav.bookStay')}
          </Link>
        </nav>
      </div>
